/*
	...
	Potential targets is either cursorTarget and/or everyone within 5m range
...
GVAR(canTakeLeadership) = {
	...
	if(isNil{aero_playtime_get}) then {
		if(rating leader _THIS(0) > rating _THIS(0)) then { 
			_out = false;
		};
	} else {
...
//[(format ["Squad Manager is active, %1, use mousewheel to bring it up",name player]), "systemChat", true, false] call BIS_fnc_MP; 

[playerSide, "HQ"] commandChat format ["%1, Squad Manager is active, T and mousewheel!",name player];
